...
function parseRangeHeader(rangeHeader, totalSize) {
  try {
    ...
    logError("Error parsing range header", error);
    return null;
  }
}
...
app.get("/api/stream/:videoId", async (req, res) => {
  ...
  try {
    ...
    if (rangeHeader && totalSize && Number.isFinite(totalSize)) {
      ...
      if (range) {
        log(`Range request: ${range.start}-${range.end}/${totalSize}`);
        res.status(206);
        res.setHeader(
          "Content-Range",
        ...
        stream.on("error", (err) => {
          logError("Stream error (range request)", err);
          if (!res.headersSent) {
            res.status(500).end();
          }
        });
        ...
        stream.on("end", () => {
          log(`Stream completed (range): ${videoId}`);
        });
    ...
    if (totalSize && Number.isFinite(totalSize)) {
      ...
      log("Streaming without content length (live or unknown size)");
    }
    ...
    stream.on("error", (err) => {
      logError("Stream error (full request)", err);
      if (!res.headersSent) {
        res.status(500).end();
      }
    });
