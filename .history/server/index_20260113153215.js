...
app.get("/api/stream/:videoId", async (req, res) => {
  ...
  try {
    ...
    // IMPORTANT: Just proxy the direct URL instead of streaming through ytdl
    // This avoids the 403 error and lets the browser handle range requests
    if (format.url) {
      log(`Redirecting to direct URL`);
      ...
      // The browser will handle range requests directly with YouTube
      return res.redirect(302, format.url);
    }

    // Fallback: Stream through our server (if redirect doesn't work)
