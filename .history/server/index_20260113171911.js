...
// IMPROVED: Fetch stream URL with deduplication
  ...
  const promise = (async () => {
    try {
      ...
      const { stdout } = await runYtDlp(
        ...
        { timeoutMs: 25_000 } // Slightly shorter timeout
      );
...
// IMPROVED: Video info endpoint with caching
...
// IMPROVED: Stream endpoint with better caching and error handling
app.get("/api/stream/:videoId", async (req, res) => {
  ...
  log(`Stream request for video: ${videoId} (Range: ${req.headers.range || "none"})`);
...
// IMPROVED: HEAD request with same caching logic
...
// IMPROVED: Cache management endpoints
...
// IMPROVED: Periodic cache cleanup
setInterval(() => {
  const now = Date.now();
  ...
  if (streamsCleaned > 0 || infoCleaned > 0) {
    log(`Cache cleanup: removed ${streamsCleaned} streams, ${infoCleaned} info entries`);
  }
