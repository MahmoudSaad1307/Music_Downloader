...
async function fetchStreamUrl(videoId) {
  ...
  const promise = (async () => {
    try {
      ...
      // OPTIMIZED: Prioritize faster-loading formats
      const { stdout } = await runYtDlp(
        [
          "-f",
          "bestaudio[ext=m4a][filesize<50M]/bestaudio[ext=mp3]/bestaudio[ext=webm]/bestaudio/best",
          "-g",
          "--no-playlist",
          "--no-warnings",
          "--extractor-args",
          "youtube:player_client=android,web",
          "--throttled-rate", "100K",  // Don't wait for slow formats
