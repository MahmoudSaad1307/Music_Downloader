import { ListMusic, Pause, Play, Plus, Search, SkipBack, SkipForward, Trash2, Volume2, X } from "lucide-react";
...
export default function App() {
  ...
  const streamUrl = useMemo(() => {
    ...
    return `/api/stream/${encodeURIComponent(currentSong.videoId)}`;
  }, [currentSong?.videoId]);
  ...
  async function toSong(videoId) {
    const res = await fetch(`/api/info/${encodeURIComponent(videoId)}`);
  ...
  async function playNowFromResult(result) {
    try {
      ...
      setSearchError("Could not load video info.");
    }
  }
    ...
    <div className="min-h-screen pb-32">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-2xl font-semibold tracking-tight">YouTube Audio Player</div>
            <div className="text-sm text-slate-400">Search, queue, and play audio only.</div>
          </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            {searchError ? <div className="mt-3 text-sm text-rose-400">{searchError}</div> : null}
                ...
                >
                  <div className="flex gap-3 p-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="truncate text-xs text-slate-400">{r.channelName}</div>
                    ...
                    return (
                      <div
                        ...
                        className={[
                          ...
                          active ? "border-indigo-500 bg-indigo-500/10" : "border-slate-800 bg-slate-950/30",
                        ].join(" ")}
                      >
                        <button
                          type="button"
                        ...
                        >
                          <img src={s.thumbnail} alt="" className="h-10 w-10 rounded-md object-cover" />
                          <div className="min-w-0">
        ...
        <div className="mx-auto max-w-6xl px-4 py-4">
          {!currentSong ? (
            <div className="flex items-center justify-between gap-4">
              <div className="text-sm text-slate-400">Pick a video to start playing.</div>
            ...
            <div className="grid gap-3 lg:grid-cols-[1fr_420px_1fr] lg:items-center">
              <div className="flex min-w-0 items-center gap-3">
                <img src={currentSong.thumbnail} alt="" className="h-12 w-12 rounded-lg object-cover" />
                <div className="min-w-0">
                  <div className="truncate text-sm font-semibold">{currentSong.title}</div>
                  <div className="truncate text-xs text-slate-400">{currentSong.channelName}</div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-12 text-right text-xs tabular-nums text-slate-400">{formatTime(currentTime)}</div>
                  <input
                    type="range"
                    min={0}
                    max={duration || 0}
                    ...
                    className="w-full"
                  />
                  <div className="w-12 text-xs tabular-nums text-slate-400">{formatTime(duration)}</div>
                </div>
              </div>
