...
export default function App() {
  ...
  async function playNowFromResult(result) {
    ...
    toSong(result.videoId)
      ...
      .catch((err) => {
        setSearchError(err instanceof Error ? err.message : "Could not load video.");
        flushSync(() => {
          setPlaylist([]);
          setCurrentSongIndex(0);
          setIsPlaying(false);
  ...
  function addToQueueFromResult(result) {
    ...
    flushSync(() => {
      setPlaylist([...currentPlaylist, tempSong]);
    });
    ...
    toSong(result.videoId)
      .then((song) => {
        setPlaylist((current) =>
          current.map((s) => (s.videoId === song.videoId ? song : s))
        );
      })
      .catch((err) => {
        setSearchError(err instanceof Error ? err.message : "Could not load video.");
        setPlaylist((current) =>
          current.filter((s) => s.videoId !== result.videoId)
        );
      });
  }
  ...
  useEffect(() => {
    ...
    const onError = () => setPlayerError("Audio failed to load. Try another video.");
